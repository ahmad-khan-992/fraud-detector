...
function EmptyState() {
  return (
    <div className="flex flex-col items-center gap-5 py-24 text-center">
      ...
      <div>
        <p className="text-base font-semibold text-slate-600">No saved sessions</p>
        <p className="text-sm text-slate-400 mt-1 max-w-xs">
          Run an audit and click "Save Session" in the top bar to store results here.
        </p>
      </div>
    </div>
  )
...
export default function SavedSessionsPage() {
  ...
  if (savedSessions.length === 0) return <EmptyState />
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            {savedSessions.length} session{savedSessions.length !== 1 ? 's' : ''} stored · Last 5 kept automatically
          </p>
        </div>
      </div>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-4 mt-2 flex-wrap">
                  <span className="text-xs text-slate-500">
                    <span className="font-semibold text-slate-800">{total.toLocaleString()}</span> rows
                  </span>
                  <span className="text-xs text-slate-500">
                    <span className="font-semibold text-amber-700">{flagged}</span> flagged
                  </span>
                  <span className={`text-xs font-semibold ${rateColor}`}>
                    {riskPercent}% flag rate
                  </span>
                  {riskCounts.High > 0 && (
                  ...
                  <div className="flex gap-3 mt-1.5">
                    {['High', 'Medium', 'Low'].map(level => (
                      <div key={level} className="flex items-center gap-1">
                        ...
                        <span className="text-xs text-slate-400">{level}: {riskCounts[level] || 0}</span>
                      </div>
                    ))}
                  </div>
                </div>
