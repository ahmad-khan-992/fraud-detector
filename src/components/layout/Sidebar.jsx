  ...
  return (
    <aside className="fixed inset-y-0 left-0 w-60 bg-slate-900 flex flex-col z-20 no-print">
      ...
      <div className="flex items-center gap-3 px-5 h-16 border-b border-white/10 shrink-0">
        ...
        <div>
          <p className="text-sm font-bold text-white leading-tight">AuditIQ</p>
          <p className="text-xs text-slate-400">Fraud Analytics Platform</p>
        </div>
      </div>
        ...
        {NAV.map(({ to, label, icon }) => (
          ...
          >
            {({ isActive }) => (
              <>
                {isActive && (
                  <span className="absolute left-0 inset-y-0 w-0.5 bg-brand-500 rounded-full" />
                )}
                {icon}
                <span>{label}</span>
      ...
      <div className="px-5 py-4 border-t border-white/10 shrink-0">
        <p className="text-xs text-slate-500 font-medium">AuditIQ · Phase 4</p>
