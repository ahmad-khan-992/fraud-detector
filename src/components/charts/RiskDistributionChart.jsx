...
const COLORS = { Critical: '#7f1d1d', High: '#ef4444', Medium: '#f59e0b', Low: '#10b981' }
  ...
  const CustomTooltip = ({ active, payload }) => {
    ...
    return (
      <div className="bg-white border border-slate-200 rounded-lg px-3 py-2 shadow-md text-xs">
        ...
        <p className="text-slate-500 mt-0.5">{value} {value === 1 ? t('analyticsPage.flagSingular') : t('analyticsPage.flagPlural')}</p>
      </div>
    )
  }

  const data = ['Critical', 'High', 'Medium', 'Low']
    .map(level => ({ name: t('risk.' + level), level, value: riskCounts[level] || 0 }))
    .filter(d => d.value > 0)

  if (data.length === 0) {
    return (
      <div className="flex items-center justify-center h-56 text-sm text-slate-400">
        {t('analyticsPage.benfordNoData')}
      </div>
    )
  }
